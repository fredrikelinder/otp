...
start(Config) when is_list(Config) ->
    ...
    %% async call
    ?line {ok, Pid8} = gen_server:start(gen_server_SUITE, [], []),
    ?line Mref8a = gen_server:async_call(Pid8, started_p),
    ?line receive
              {Mref8a, ok} ->
                  ok
          after 5000 ->
    ...
    ?line Mref8b = gen_server:async_call(Pid8, stop_p),
    ?line receive
              {Mref8b, ok} ->
                  ok
          after 5000 ->
    ...
    ?line Mref8c = gen_server:async_call(Pid8, stop_p),
    ?line receive
              {'DOWN', Mref8c, Pid8, process, noproc} ->
                  ok
          after 5000 ->
                  test_server:fail(not_monitored)
          end,
